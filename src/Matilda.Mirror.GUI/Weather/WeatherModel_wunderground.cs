...
namespace Matilda.Mirror.GUI.Weather
{
    internal class WeatherModel_wunderground : WeatherModel
    {
        ...
        //public int Snowfall { get; private set; }
        ...
        {
            var requestHourly = WebRequest.Create("http://api.wunderground.com/api/49b92ea52ed55fe0/hourly/q/NY/New_York.json");
            ...
            var request10Day = WebRequest.Create("http://api.wunderground.com/api/49b92ea52ed55fe0/forecast10day/q/NY/New_York.json");
        ...
        private void updateWithHourlyData(string response)
        {
            ...
            {
                var rawEpoch = Int32.Parse(hourly["FCTTIME"]["epoch"].ToString());
                var epoch = DateTimeOffset.FromUnixTimeSeconds(rawEpoch); 
                var forecast = new WeatherDetailsModel()
                {
